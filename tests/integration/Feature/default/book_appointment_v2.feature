Feature: Create booking
  ...
  Background:
    ...
    And the working day "7fd1ad2c-05b1-4814-af37-d74d81a69f1d" has bookings:
      ...
      | men_haircut | 9:00  | John            | Doe            | john.doe@test.com    |
      | combo       | 10:30 | Jane            | Dawson         | jane.dawson@test.com |

  Scenario: Happy path
    When I send a POST request to "v2/working-days/7fd1ad2c-05b1-4814-af37-d74d81a69f1d/bookings" with body:
    """
      {
          "time": "9:00",
          "service": "men_haircut",
  ...
  Scenario: Validation - cannot be booked
    ...
    """
    Then the response code is 204
    And the email to "john.doe@test.com" has been sent

  Scenario: Validation - schema
    When I send a POST request to "v2/working-days/123/bookings" with body:
    """
      {
          "time": "123",
          "service": "invalid",
